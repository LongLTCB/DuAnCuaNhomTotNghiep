...
public static class ThuatToanMaNhiPhan
{
    ...
    public static bool TryResolveBasicShape(int type, out WallShape shape)
    {
        if (wallTop.Contains(type))
        {
            shape = WallShape.Top;
            return true;
        }

        if (wallSideLeft.Contains(type))
        {
            shape = WallShape.Left;
            return true;
        }

        if (wallSideRight.Contains(type))
        {
            shape = WallShape.Right;
            return true;
        }

        if (wallBottm.Contains(type))
        {
            shape = WallShape.Bottom;
            return true;
        }

        if (wallFull.Contains(type))
        {
            shape = WallShape.Full;
            return true;
        }
    ...
    public static bool IsCornerMask(int type)
    {
        return wallInnerCornerDownLeft.Contains(type)
               || wallInnerCornerDownRight.Contains(type)
               || wallDiagonalCornerDownLeft.Contains(type)
               || wallDiagonalCornerDownRight.Contains(type)
               || wallDiagonalCornerUpLeft.Contains(type)
               || wallDiagonalCornerUpRight.Contains(type)
               || wallFullEightDirections.Contains(type)
               || wallBottmEightDirections.Contains(type);
    }
